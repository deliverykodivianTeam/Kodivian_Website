  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-700 via-purple-600 to-pink-500 relative overflow-hidden">
      ...
      <div className="relative bg-white/25 backdrop-blur-lg border border-white/40 shadow-2xl p-10 rounded-3xl w-96 text-center animate-fade-in">
        ...
        <p className="text-white/90 mb-6 text-sm">Secure access to visitor insights</p>
        <form onSubmit={handleLogin} className="space-y-4">
          <input
            ...
            required
          />
          <button type="submit" disabled={loading} className="w-full py-3 rounded-lg bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold hover:from-purple-600 hover:to-indigo-500 transition-all">
            {loading ? "Logging in..." : "Login"}
          </button>
        </form>
        {error && <p className="text-red-200 mt-4 text-sm font-medium">{error}</p>}
        <p className="text-xs text-white/70 mt-6">© {new Date().getFullYear()} Visitor Analytics Dashboard</p>
      </div>
    </div>
  );
}
