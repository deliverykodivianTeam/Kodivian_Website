...
function VisitorDetails() {
  ...
  const fetchVisitors = async () => {
    try {
      ...
      setVisitors(res.data.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)));
    } catch (err) {
  ...
  const handleDeleteVisitor = async (timestamp) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/delete/${timestamp}`);
      setVisitors(visitors.filter((v) => v.timestamp !== timestamp));
    } catch (err) {
  ...
  if (!visitors.length) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gradient-to-br from-indigo-200 via-purple-100 to-pink-100">
        <h2 className="text-2xl text-indigo-800 font-semibold mb-2">No visitor data found</h2>
        <p className="text-gray-600">Try visiting the website to generate records.</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-200 via-purple-100 to-pink-100 py-10 px-4">
      <div className="max-w-6xl mx-auto bg-white shadow-2xl rounded-3xl p-8 border border-indigo-100">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-indigo-800">👥 Website Visitor Records</h1>
          <button
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-6 rounded-2xl mb-8 shadow-md">
          ...
          <div className="grid grid-cols-2 gap-4">
            <p><b>IP:</b> {visitors[0].ip}</p>
            <p><b>City:</b> {visitors[0].city}</p>
            <p><b>Country:</b> {visitors[0].country}</p>
            <p><b>Visited:</b> {visitors[0].timestamp}</p>
          </div>
        </div>
          ...
          <table className="w-full text-center border border-gray-200 rounded-xl overflow-hidden">
            ...
            <tbody>
              {visitors.map((v, index) => (
                <tr key={v.timestamp} className={`${index % 2 === 0 ? "bg-gray-50" : "bg-white"} hover:bg-indigo-50 transition-all duration-200`}>
                  <td className="p-3 text-indigo-800 font-semibold">{index + 1}</td>
